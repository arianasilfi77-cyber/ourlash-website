...
function NavLink({ href, children, onClick }) {
  ...
  return href.startsWith('/') ? <Link onClick={onClick} to={href} className={classes}>{children}</Link> : <a onClick={onClick} href={href} className={classes}>{children}</a>;
}

export function LanguageSwitch({ language, setLanguage }) {
  return <div className="flex rounded-full border border-divider bg-white p-1" aria-label="Language switch">
    {Object.entries(languages).map(([code, label]) => <button key={code} onClick={() => setLanguage(code)} className={(language === code ? 'bg-navy text-white' : 'text-muted hover:text-navy') + ' rounded-full px-3 py-2 text-xs font-semibold transition'}>{label}</button>)}
  </div>;
}

export function Navigation({ navItems, language, setLanguage, bookLabel }) {
  ...
  return <header className={(scrolled ? 'bg-white/90 border-divider backdrop-blur' : 'bg-transparent border-transparent') + ' fixed inset-x-0 top-0 z-50 border-b transition duration-300'}>
    <nav aria-label="Primary" className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 md:px-8">
      ...
      <div className="hidden items-center gap-6 lg:flex">
        {navItems.map(([label, href]) => <NavLink key={label} href={href}>{label}</NavLink>)}
    ...
    {open && <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} transition={{ duration: 0.3 }} className="fixed inset-0 bg-white p-8 lg:hidden">
      ...
      <div className="mt-20 grid gap-8 text-2xl text-navy">
        {navItems.map(([label, href]) => <NavLink onClick={() => setOpen(false)} key={label} href={href}>{label}</NavLink>)}
        <LanguageSwitch language={language} setLanguage={setLanguage} />
        <Button>{bookLabel}</Button>
      </div>
    </motion.div>}
